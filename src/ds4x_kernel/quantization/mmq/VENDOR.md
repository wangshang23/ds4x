...
cd /path/to/ds4x/src/ds4x_kernel/quantization/mmq
...
# Update this VENDOR.md's commit pin and run `make kernel-test` on DGX Spark.
```
...
| Test                                          | Status                       | Phase     |
|-----------------------------------------------|------------------------------|-----------|
| `nvcc -c src/ds4x_kernel/quantization/mmq/ds4_mmq.cu` builds cleanly | **passes** (sm_120, nvcc 13) | Phase 0 |
...
Sustained ~2.80x prefill speedup across the swept context range; generation
remained within 2.5% of baseline (run-to-run variance). Standalone parity
probes for the adapted kernels live in `tests/ds4x_kernel/mmq/`.
