...
static int check_packed_batch_attention_fp16_tokentile_parity(void) {
    const uint32_t n_tokens = 128u;
    const uint32_t pos0 = 4096u;
    const uint32_t n_head = 64u;
    const uint32_t head_dim = 512u;
    const uint32_t n_raw = n_tokens + 127u;
    const uint32_t raw_cap = n_raw;
    const uint32_t raw_start = 37u;
    const uint32_t top_k = 512u;
    const uint32_t max_comp = (pos0 + n_tokens) / 4u;
    const uint64_t q_stride = (uint64_t)n_head * head_dim;
    const uint64_t q_count = (uint64_t)n_tokens * q_stride;
    ...
    float *ref_host = malloc((size_t)q_count * sizeof(*ref_host));
    float *packed_host = malloc((size_t)q_count * sizeof(*packed_host));
    uint32_t *topk_host = malloc((size_t)n_tokens * top_k * sizeof(*topk_host));
    if (!sinks || !q_host || !raw_host || !comp_host ||
        !ref_host || !packed_host || !topk_host) {
        free(topk_host);
        free(packed_host);
        free(ref_host);
        free(comp_host);
    ...
    for (uint64_t i = 0; i < q_count; i++) {
        q_host[i] = qat_value(i + 131u) * 0.0625f;
    }
    for (uint64_t i = 0; i < raw_count; i++) {
        raw_host[i] = qat_value(i + 1217u);
    }
    for (uint64_t i = 0; i < comp_count; i++) {
        comp_host[i] = qat_value(i + 12011u);
    }
    for (uint32_t t = 0; t < n_tokens; t++) {
        for (uint32_t k = 0; k < top_k; k++) {
    ...
    ds4_gpu_tensor *raw_src = ds4_gpu_tensor_alloc(raw_count * sizeof(float));
    ds4_gpu_tensor *comp_src = ds4_gpu_tensor_alloc(comp_count * sizeof(float));
    ds4_gpu_tensor *raw_f32 = ds4_gpu_tensor_alloc(raw_count * sizeof(float));
    ...
    ds4_gpu_tensor *comp_packed = ds4_gpu_tensor_alloc(
        (uint64_t)max_comp * DS4_SPARK_KV_ROW_BYTES);
    ds4_gpu_tensor *topk = ds4_gpu_tensor_alloc(
        (uint64_t)n_tokens * top_k * sizeof(uint32_t));
    ds4_gpu_tensor *heads_ref = ds4_gpu_tensor_alloc(q_count * sizeof(float));
    ds4_gpu_tensor *heads_packed = ds4_gpu_tensor_alloc(q_count * sizeof(float));
    int rc = 1;
    if (!q || !raw_src || !comp_src || !raw_f32 || !comp_f32 ||
        !raw_packed || !comp_packed || !topk || !heads_ref || !heads_packed ||
        !ds4_gpu_tensor_write(q, 0, q_host, q_count * sizeof(float)) ||
        !ds4_gpu_tensor_write(raw_src, 0, raw_host, raw_count * sizeof(float)) ||
        !ds4_gpu_tensor_write(comp_src, 0, comp_host, comp_count * sizeof(float)) ||
        !ds4_gpu_tensor_write(topk, 0, topk_host,
                              (uint64_t)n_tokens * top_k * sizeof(uint32_t)) ||
        !ds4_gpu_spark_pack_kv_rows_tensor(
            raw_packed, 0, raw_src, 0, raw_cap) ||
        !ds4_gpu_spark_pack_kv_rows_tensor(
            comp_packed, 0, comp_src, 0, max_comp) ||
        !ds4_gpu_spark_unpack_kv_rows_tensor(raw_f32, raw_packed, raw_cap) ||
        !ds4_gpu_spark_unpack_kv_rows_tensor(comp_f32, comp_packed, max_comp)) {
        goto cleanup;
    }

    unsetenv("DS4_CUDA_SPARK_PREFILL_EXACT");
    const uint32_t dense_comp = (pos0 + n_tokens) / 128u;
    if (!ds4_gpu_attention_decode_mixed_batch_heads_tensor(
            heads_ref, sinks, n_head * sizeof(float), 0,
            q, raw_f32, comp_f32, DS4_GPU_CACHE_F32, NULL, 0,
            n_tokens, pos0, n_raw, raw_cap, raw_start, dense_comp,
            128u, 128u, n_head, head_dim) ||
        !ds4_gpu_attention_decode_mixed_batch_heads_tensor(
            heads_packed, sinks, n_head * sizeof(float), 0,
            q, raw_packed, comp_packed, DS4_GPU_CACHE_SPARK_KV, NULL, 0,
            n_tokens, pos0, n_raw, raw_cap, raw_start, dense_comp,
            128u, 128u, n_head, head_dim) ||
        !ds4_gpu_synchronize() ||
        !ds4_gpu_tensor_read(heads_ref, 0, ref_host,
    ...
    for (uint64_t i = 0; i < q_count; i++) {
        const float delta = fabsf(ref_host[i] - packed_host[i]);
        if (delta > dense_max_abs) dense_max_abs = delta;
    }

    if (!ds4_gpu_attention_indexed_mixed_batch_heads_tensor(
            heads_ref, sinks, n_head * sizeof(float), 0,
            q, raw_f32, comp_f32, DS4_GPU_CACHE_F32, topk,
            n_tokens, pos0, n_raw, raw_cap, raw_start, max_comp, top_k,
            128u, 4u, n_head, head_dim) ||
        !ds4_gpu_attention_indexed_mixed_batch_heads_tensor(
            heads_packed, sinks, n_head * sizeof(float), 0,
            q, raw_packed, comp_packed, DS4_GPU_CACHE_SPARK_KV, topk,
            n_tokens, pos0, n_raw, raw_cap, raw_start, max_comp, top_k,
            128u, 4u, n_head, head_dim) ||
        !ds4_gpu_synchronize() ||
        !ds4_gpu_tensor_read(heads_ref, 0, ref_host,
    ...
    float indexed_max_abs = 0.0f;
    for (uint64_t i = 0; i < q_count; i++) {
        const float delta = fabsf(ref_host[i] - packed_host[i]);
        if (delta > indexed_max_abs) indexed_max_abs = delta;
    }
    fprintf(stderr,
            "cuda-regression: packed FP16 token-tile dense_max_abs=%g indexed_max_abs=%g\n",
            (double)dense_max_abs, (double)indexed_max_abs);
    rc = dense_max_abs <= 1.0e-5f && indexed_max_abs <= 1.0e-5f ? 0 : 1;

cleanup:
    ds4_gpu_tensor_free(heads_packed);
    ds4_gpu_tensor_free(heads_ref);
    ds4_gpu_tensor_free(topk);
    ds4_gpu_tensor_free(comp_packed);
...
int main(void) {
    ...
    if (check_packed_batch_attention_fp16_tokentile_parity() != 0) rc = 1;
